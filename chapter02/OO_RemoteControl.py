class TV:
    ...
    def power(self):
        """Method that toggles the self.isOn Boolean"""
        self.isOn = not self.isOn  # toggle

    def volumeUp(self):
        """Method checks the TV is on, if not exits. if the volume is lower than the
        maximum volume increment self with 1"""
        if not self.isOn:
            return
        if self.isMuted:
    ...
    def volumeDown(self):
        """Method checks the TV is on, if not exits. if the volume is higher than the
        minimum volume decrease self with 1"""
        if not self.isOn:
            return
        if self.isMuted:
            self.isMuted = False  # changing the volume while muted unmutes the sound
    ...
    def channelDown(self):
        ...
        if self.channelIndex < 0:
            self.channelIndex = -1  # wrap around to the top channel
...
# Main code
oTV = TV("Samsung", "Family room")  # create the TV object
